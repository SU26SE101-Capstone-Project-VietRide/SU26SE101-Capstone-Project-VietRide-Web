...
/**
 * Nhật ký hoạt động toàn hệ thống (`GET /v1/admin/activity-logs`).
 *
 * Spec BE 2026-08-25 vẫn CHỈ trả `actor` + `action` + `metadata` thô, chưa có
 * snapshot tên đối tượng mà FE đã đề nghị trong
 * `docs/BE_ACTIVITY_LOG_READABLE_TARGET_CONTRACT.md`. Vì vậy bảng cố tình đẩy
 * mọi UUID xuống modal chi tiết: hàng chỉ hiện thời gian, người thực hiện và mã
 * tác vụ — những thứ đọc được — còn UUID và `metadata` để dành cho lúc điều tra.
 *
 * `data.items` có thể là `null` theo spec, nên chuẩn hoá về `[]` ngay tại chỗ
 * nhận thay vì rải `?? []` khắp phần render.
 */
export default function ActivityLogs() {
  ...
  const [selected, setSelected] = useState<AdminActivityLogItem | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        subtitle={selected?.action ?? undefined}
        footer={
          <button
            type="button"
