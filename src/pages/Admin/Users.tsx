...
function UserAvatar({
  ...
  const sizeClass = large
    ? "h-[88px] w-[88px] rounded-2xl text-2xl"
  ...
  if (user.avatarUrl) {
    return (
      <img
        ...
        height={large ? 88 : 40}
        loading="lazy"
        className={`${sizeClass} shrink-0 ${large ? "border-4 border-white shadow-md" : "border border-gray-200"} bg-white object-cover`}
      />
    );
  }
  ...
  return (
    <div
      aria-label={name}
      className={`${sizeClass} inline-flex shrink-0 items-center justify-center bg-gradient-to-br font-bold text-white shadow-sm ${avatarGradientByRole[user.role]}`}
    >
      {getInitials(name)}
    </div>
  );
...
export default function Users() {
  ...
  const [pendingLockUser, setPendingLockUser] = useState<AdminUser | null>(
    null,
  );
  ...
  useToastFeedback({
    message: message?.tone === "success" ? message.text : "",
    error: message?.tone === "error" ? message.text : "",
  });
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <StatCard
          label={t("users.operatorStaff")}
          value={stats?.operatorStaff ?? 0}
          icon={<FiUsers size={20} />}
          iconClassName="bg-blue-50 text-blue-700"
        />
        <StatCard
          label={tc("enumLabels.PENDING_INITIAL_PASSWORD")}
      ...
      <PersonnelTable
        toolbar={
          <div className="grid gap-3 md:grid-cols-[minmax(200px,0.9fr)_minmax(180px,1.1fr)_minmax(190px,1.1fr)_minmax(200px,1.2fr)]">
            <SearchInput
            ...
            >
              ...
              <option value="PENDING_EMAIL_VERIFICATION">
                {tc("enumLabels.PENDING_EMAIL_VERIFICATION")}
              </option>
              <option value="PENDING_INITIAL_PASSWORD">
                {tc("enumLabels.PENDING_INITIAL_PASSWORD")}
              </option>
              <option value="DELETED">{tc("enumLabels.DELETED")}</option>
            </CustomSelect>
            <CustomSelect
              aria-label={t("users.filterOperator")}
              value={operatorId}
              onChange={(event) => {
        ...
        columns={[
          {
            ...
            headerClassName: "w-[28%] px-4 py-3 text-left",
            cellClassName: "w-[28%] px-4 py-4 text-left",
            render: (user) => (
              <div className="flex items-center gap-3">
                <UserAvatar user={user} />
                <span
                  className="min-w-0 truncate whitespace-nowrap text-sm font-semibold text-gray-900"
          ...
          },
          {
            key: "email",
            header: tc("email"),
            headerClassName: "w-[22%] px-4 py-3 text-left",
            cellClassName: "w-[22%] px-4 py-4 text-left text-sm text-gray-600",
            render: (user) => (
          ...
          },
          {
            key: "phone",
            header: tc("phone"),
            headerClassName: "w-[9%] px-4 py-3 text-center",
            cellClassName:
              "w-[9%] px-2 py-4 text-center text-sm whitespace-nowrap text-gray-600",
            render: (user) => formatVietnamPhoneForDisplay(user.phone),
          },
          {
            key: "role",
            header: t("users.role"),
            headerClassName: "w-[15%] px-4 py-3 text-center",
            cellClassName:
              "w-[15%] px-4 py-4 text-center text-sm text-gray-700",
            render: (user) => (
          ...
          },
          {
            key: "status",
            header: tc("status"),
            headerClassName: "w-[16%] px-4 py-3 text-center",
            cellClassName: "w-[16%] px-4 py-4 text-center text-sm",
            render: (user) => (
              <span
                className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ${isActiveStatus(user.status) ? "bg-emerald-50 text-emerald-700" : isLockedStatus(user.status) ? "bg-rose-50 text-rose-700" : "bg-slate-100 text-slate-600"}`}
              >
                {user.status
          ...
          },
          {
            key: "actions",
            header: tc("actions"),
            headerClassName: "w-[120px] px-2 py-3 text-center",
            cellClassName:
              "sticky right-0 w-[120px] bg-white px-2 py-4 text-center",
            render: (user) => {
              const canToggle =
                ...
                <div className="mx-auto flex w-[80px] justify-center gap-2">
                  ...
                  isLockedStatus(user.status) ? (
                    <button
                      ...
                      onClick={() => setPendingLockUser(user)}
      ...
      <ConfirmModal
        ...
        message={
          pendingLockUser
            ? isLockedStatus(pendingLockUser.status)
              ? t("users.unlockConfirm", { name: pendingLockUser.displayName })
              : t("users.lockConfirm", { name: pendingLockUser.displayName })
            : ""
        }
        confirmLabel={tc("confirm")}
        cancelLabel={tc("cancel")}
        tone={
          pendingLockUser && isLockedStatus(pendingLockUser.status)
            ? "success"
            : "danger"
        }
