  ...
  return (
    <div className="space-y-6">
      ...
      {message && (
        ...
        columns={[
          { key: "name", header: t("users.fullName"), headerClassName: "w-[20%] px-4 py-3", cellClassName: "w-[20%] px-4 py-4", render: (user) => <div className="flex items-center gap-3"><img src={user.avatarUrl || fallbackAvatarUrl} alt={user.displayName || user.email} width={40} height={40} loading="lazy" className="h-10 w-10 shrink-0 rounded-lg border border-gray-200 bg-white object-cover" /><span className="whitespace-nowrap text-sm font-semibold text-gray-900">{user.displayName || "-"}</span></div> },
          { key: "email", header: tc("email"), headerClassName: "w-[24%] px-4 py-3", cellClassName: "w-[24%] px-4 py-4 text-sm text-gray-600", render: (user) => <span className="block truncate">{user.email}</span> },
