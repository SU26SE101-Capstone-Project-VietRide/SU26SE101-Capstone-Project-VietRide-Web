...
export default function ManagerRouteManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await getOperatorRoutes({ page, pageSize: PAGE_SIZE, ...(search.trim() ? { search: search.trim() } : {}), ...(status ? { status } : {}) });
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm"><div className="grid gap-3 border-b border-gray-100 p-4 sm:grid-cols-[minmax(0,1fr)_220px]"><div className="relative"><FiSearch className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input aria-label={t("routeManagement.searchLabel")} value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder={t("routeManagement.searchPlaceholder")} className={`${inputClass} pl-9`} /></div><select aria-label={tc("status")} value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} className={inputClass}><option value="">{tc("all")}</option><option value="ACTIVE">{tc("active")}</option><option value="INACTIVE">{tc("inactive")}</option></select></div>
