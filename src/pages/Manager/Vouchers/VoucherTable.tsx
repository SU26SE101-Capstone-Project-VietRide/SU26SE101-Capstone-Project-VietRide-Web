...
export default function VoucherTable({ toolbar, vouchers, isLoading, onEdit, onToggle, onDelete }: VoucherTableProps) {
  ...
  return (
    <PersonnelTable
      ...
      columns={[
        { key: "code", header: t("vouchers.code"), headerClassName: "w-[19%] px-5 py-3 text-center", cellClassName: "w-[19%] whitespace-nowrap px-5 py-4 text-center font-mono text-sm font-semibold text-vr-700", render: (voucher) => voucher.code },
        { key: "name", header: t("vouchers.name"), headerClassName: "w-[20%] px-3 py-3 text-left", cellClassName: "w-[20%] min-w-0 px-3 py-4 text-left", render: (voucher) => <><p className="max-w-full truncate text-sm font-semibold leading-5 text-gray-900" title={voucher.name}>{truncateVoucherName(voucher.name)}</p><p className="whitespace-nowrap truncate text-xs text-gray-500">{tc(`voucherTypes.${voucher.type}`, { defaultValue: voucher.type })}</p></> },
        ...
        { key: "limit", header: t("vouchers.limit"), headerClassName: "w-[12%] px-3 py-3 text-center", cellClassName: "w-[12%] whitespace-nowrap px-3 py-4 text-center text-sm text-gray-700", render: (voucher) => <><p>{t("vouchers.totalLimit", { count: voucher.totalUsageLimit })}</p><p className="text-xs text-gray-500">{t("vouchers.perUserLimit", { count: voucher.perUserLimit })}</p></> },
        { key: "validity", header: t("vouchers.validity"), headerClassName: "w-[15%] px-3 py-3 text-center", cellClassName: "w-[15%] whitespace-nowrap px-3 py-4 text-center text-sm text-gray-700", render: (voucher) => <><p>{formatDate(voucher.validFrom)}</p><p className="text-xs text-gray-500">{t("vouchers.validUntil", { date: formatDate(voucher.validUntil) })}</p></> },
