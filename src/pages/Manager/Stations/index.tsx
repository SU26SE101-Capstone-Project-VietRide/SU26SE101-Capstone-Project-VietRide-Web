...
export default function ManagerStationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await getOperatorStations({
        ...
        ...(shuttleFilter
          ? { supportsShuttle: shuttleFilter === "SHUTTLE" }
          : {}),
        sortBy: "name",
        sortDir: "asc",
      });
  ...
  const shuttleCount = useMemo(
    () => items.filter((item) => item.station?.supportsShuttle).length,
    [items],
  );
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 p-4">
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_200px_220px]">
            ...
            />
            <CustomSelect
              aria-label={t("stations.filterActive")}
              value={activeFilter}
              onChange={(event) => {
            ...
            <CustomSelect
              aria-label={t("stations.filterShuttle")}
              value={shuttleFilter}
              onChange={(event) => {
            ...
            <table className="w-full min-w-[760px] table-fixed divide-y divide-gray-100 text-sm">
              <thead className="bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-500">
                <tr>
                  ...
                  <th className="w-[43%] px-5 py-3 text-left">
                    {t("stations.address")}
                  </th>
                  <th className="w-[16%] px-5 py-3 text-center whitespace-nowrap">
                    {t("stations.shuttle")}
                  </th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  ...
                  >
                    ...
                    <td
                      className="px-5 py-4 align-middle text-left text-gray-600"
                      title={stationAddress(item)}
                    >
                      <p className="break-words">{stationAddress(item)}</p>
                    </td>
                    <td className="px-5 py-4 align-middle text-center text-gray-600">
                      {item.station?.supportsShuttle ? tc("yes") : tc("no")}
                    </td>
                    <td className="px-5 py-4 align-middle text-center">
                      <Badge
                        tone={item.isActive !== false ? "success" : "neutral"}
                      >
                        {item.isActive !== false
                          ? tc("active")
                          : tc("inactive")}
