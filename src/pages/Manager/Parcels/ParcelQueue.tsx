  ...
  return (
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <PersonnelTable
        ...
        columns={[
          { key: "code", header: t("parcels.orderCode"), headerClassName: "w-[18%] px-5 py-3", cellClassName: "w-[18%] px-5 py-4", render: (item) => <><p className="font-semibold text-gray-900">{item.parcelCode}</p><p className="mt-1 text-xs text-gray-500">{formatDateTime(item.createdAt)}</p></> },
          { key: "route", header: t("parcels.queue.routeNameLabel"), headerClassName: "w-[20%] px-5 py-3", cellClassName: "w-[20%] px-5 py-4 text-sm", render: (item) => <p className="font-medium text-gray-800">{item.route?.routeName || item.routeName || t("parcels.queue.noRouteName")}</p> },
          ...
          { key: "size", header: t("parcels.sizeCategory"), headerClassName: "w-[14%] px-5 py-3", cellClassName: "w-[14%] px-5 py-4 text-sm text-gray-700", render: (item) => <>{item.sizeCategory ? t(`parcels.sizeCategories.${item.sizeCategory}`, { defaultValue: item.sizeCategory }) : "-"}<br /><span className="text-xs text-gray-500">{item.estimatedWeightKg == null ? "-" : `${item.estimatedWeightKg} kg`}</span></> },
          ...
          { key: "actions", header: tc("actions"), headerClassName: "w-[120px] px-5 py-3 text-right", cellClassName: "w-[120px] px-5 py-4 text-right", render: (item) => <button type="button" onClick={() => void openDetail(item)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-700 hover:border-vr-300 hover:text-vr-700">{t("parcels.queue.openAction")}</button> },
        ]}
        rows={items}
