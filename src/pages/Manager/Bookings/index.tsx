...
function BookingDetailContent({ booking, statusBadge }: BookingDetailContentProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Bảng ghế phía trên là ghế CỦA LƯỢT ĐẶT NÀY; panel dưới là toàn bộ ghế
          của chuyến, để nhà xe thấy chỗ trống/đang giữ mà không phải mở app. */}
      {booking.tripId ? <TripSeatMapPanel tripId={booking.tripId} /> : null}

      <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm"><h3 className="text-base font-semibold text-gray-900">{t("bookings.statusTimeline")}</h3>{timeline.length === 0 ? <p className="mt-4 rounded-lg bg-gray-50 p-4 text-sm text-gray-500">{t("bookings.noTimeline")}</p> : <div className="relative mt-4 space-y-4 pl-5 before:absolute before:bottom-2 before:left-[7px] before:top-2 before:w-px before:bg-gray-200">{timeline.map((entry, index) => <div key={`${entry.occurredAt}-${index}`} className="relative flex gap-3"><span className="absolute -left-5 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-vr-500 shadow-sm" /><div className="min-w-0 flex-1 rounded-lg border border-gray-100 bg-gray-50/70 p-3"><div className="flex flex-wrap items-center justify-between gap-2">{statusBadge(entry.status)}<time className="text-xs text-gray-500">{formatDateTime(entry.occurredAt)}</time></div>{entry.reasonCode && <p className="mt-2 text-sm text-gray-600">{t("bookings.reasonCode")}: {t(`bookings.cancelReasons.${entry.reasonCode}`, { defaultValue: entry.reasonCode })}</p>}</div></div>)}</div>}{booking.cancellationReason && <div className="mt-4 rounded-lg border border-red-100 bg-red-50 p-3 text-sm text-red-800"><span className="font-semibold">{t("bookings.cancellationReason")}:</span>{" "}{t(`bookings.cancelReasons.${booking.cancellationReason}`, { defaultValue: booking.cancellationReason })}</div>}</section>
    </div>
  );
}
