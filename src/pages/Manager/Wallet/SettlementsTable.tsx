              ...
              <SettlementRowGroup
                ...
                onToggle={() =>
                  setExpandedId((current) =>
                    current === item.settlementId ? null : item.settlementId,
                  )
                }
              />
            ))
    ...
    <>
      ...
      >
        ...
        <td className="overflow-hidden px-4 py-3">
          ...
          {item.trip?.routeName && (
            <span
              title={item.trip.routeName}
              className="mt-0.5 block max-w-[220px] truncate text-xs text-gray-500"
            >
              {item.trip.routeName}
            </span>
          )}
...
function SettlementStateDetail({
  item,
  t,
  tc,
}: {
  ...
  return (
    <div className="mb-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-600">
      {state === "ON_HOLD" && item.eligibleAt && (
        <span>
          {t("wallet.eligibleAt")}: {formatWalletDate(item.eligibleAt)}
        </span>
      )}
      {state === "RETRY_SCHEDULED" && (
        <>
          {item.delayReason && (
            <span>
              {t(`wallet.delayReason.${item.delayReason}`, {
                defaultValue: t("wallet.delayReason.default"),
              })}
            </span>
          )}
          {item.attemptCount !== undefined && (
            <span>
              {t("wallet.attemptCount", { count: item.attemptCount })}
            </span>
          )}
          {item.lastAttemptAt && (
            <span>
              {t("wallet.lastAttemptAt")}:{" "}
              {formatWalletDate(item.lastAttemptAt)}
            </span>
          )}
          {item.nextRetryAt && (
            <span>
              {t("wallet.nextRetryAt")}: {formatWalletDate(item.nextRetryAt)}
            </span>
          )}
        </>
      )}
      {state === "COMPLETED" && (
        <>
          {item.settledAt && (
            <span>
              {t("wallet.settledAt")}: {formatWalletDate(item.settledAt)}
            </span>
          )}
          {actorDisplayName(item.settledBy, tc) && (
            <span>
              {t("wallet.settledBy")}: {actorDisplayName(item.settledBy, tc)}
            </span>
          )}
        </>
      )}
      ...
      {state === "CANCELLED" && item.cancelReason && (
        <span>
          {t("wallet.cancelReason")}:{" "}
          {t(`wallet.cancelReasons.${item.cancelReason}`, {
            defaultValue: item.cancelReason,
          })}
        </span>
      )}
      <DataCompletenessBadge
        completeness={item.trip === null ? "PARTIAL" : undefined}
        t={t}
      />
    </div>
  );
...
function SettlementBreakdown({
  ...
  const hasBreakdown = BREAKDOWN_FIELDS.some(
