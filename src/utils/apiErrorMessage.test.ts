...
describe("translateApiErrorMessage", () => {
  ...
  it("returns the fallback untouched when language is not Vietnamese", async () => {
    ...
    expect(
      translateApiErrorMessage("BOOKING_NOT_FOUND", "Booking not found"),
  ...
  it.each([
    ["The Reason field is required.", "Vui lòng nhập lý do."],
    ...
    [
      ...
      "Số dòng mỗi trang phải nằm trong khoảng 1 đến 100.",
    ],
  ])("translates the FluentValidation message %s", async (raw, expected) => {
    await i18n.changeLanguage("vi");
  ...
  it("keeps the English field name when it is not in the dictionary yet", async () => {
    ...
    expect(
      translateApiErrorMessage(
        undefined,
        "'Some New Field' must not be empty.",
      ),
  ...
  it("uses the generic 401/403/5xx translation when status is given but code is missing/unmapped", async () => {
    await i18n.changeLanguage("vi");
    expect(translateApiErrorMessage(undefined, "Unauthorized", 401)).toBe(
